...
public class StarrSearchRescueIssueCertificate extends BaseClass implements API
{
	...
	{
		request = new JsonHandle(config.getProperty("request_location")+input.ReadData("Test_case_id")+"_request"+".json");
	...
	public void SendAndReceiveData() throws SQLException
	{
		...
		response = new JsonHandle(config.getProperty("response_location")+input.ReadData("Test_case_id")+"_response"+".json");
		try {
