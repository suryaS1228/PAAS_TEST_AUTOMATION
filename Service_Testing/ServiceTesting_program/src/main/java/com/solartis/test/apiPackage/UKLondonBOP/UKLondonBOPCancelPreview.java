...
public class UKLondonBOPCancelPreview extends BaseClass implements API 
{
	MacroInterface macro = null;
	public UKLondonBOPCancelPreview (PropertiesHandle config) throws MacroException
	{
		this.config = config;
		...
		{
		macro=new UKLondonBOPCancelPreviewMacro(config);
		}
...
@Override
	public String tokenGenerator(PropertiesHandle config)
	{
		...
		try
		{
		   ...
		   String input_data = "{  \"ServiceRequestDetail\": { \"OwnerId\": \""+config.getProperty("OwnerID")+"\", \"ResponseType\": \"JSON\", \"BrowserIp\": \"127.0.0.1\", \"ServiceRequestVersion\": \"2.0\" }, \"UserCredential\": { \"UserName\": \""+config.getProperty("Userneme")+"\",    \"Password\": \""+config.getProperty("Password")+"\"  } }";
