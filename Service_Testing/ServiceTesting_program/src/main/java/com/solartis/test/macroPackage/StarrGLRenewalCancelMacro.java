...
public class StarrGLRenewalCancelMacro extends DBColoumnVerify implements MacroInterface
{
	protected ExcelOperationsPOI sampleexcel=null;
	protected String Targetpath;
	protected StarrGLRenewalCancelMacro trans;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	public StarrGLRenewalCancelMacro()
	{
		
	}
	public StarrGLRenewalCancelMacro(PropertiesHandle configFile) throws MacroException, DatabaseException
	{
	...
	public void PumpinData(LinkedHashMap<String, String> inputData,PropertiesHandle configFile) throws MacroException
	{
		try
		{
			...
			trans= new StarrGLRenewalCancelMacro(configFile);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	...
	public static void main(String args[]) throws PropertiesHandleException, DatabaseException, MacroException
	{
		...
		StarrGLRenewalCancelMacro MG;
		PropertiesHandle configFile=null;
		
		configFile = new PropertiesHandle("R:\\RestFullAPIDeliverable\\Devolpement\\admin\\STARR-GL\\RenewalCancel\\Config\\config.properties");
		objectInput.ConnectionSetup(configFile);
		System.out.println(configFile.getProperty("input_query"));
		 ...
		 while (inputtableiterator.hasNext() && outputtableiterator.hasNext()) 
			{
			 System.out.println("coming to flow4");
		        ...
		        System.out.println("Flag is"+inputrow.get("Flag_for_execution"));
		        if(inputrow.get("Flag_for_execution").equals("Y"))
				{
					System.out.println("coming to flow");
					MG=new StarrGLRenewalCancelMacro(configFile);
